...
function addPolygonAtLocation(latitude, longitude) {
    ...
    for (var i = 0; i < numberOfPoints; i++) {
        ...
        boundaries.push(new WorldWind.Position(latitude + offsetLat, longitude + offsetLon, 400000.0));
    }
...
function shareOnFacebook() {
    ...
    const title = encodeURIComponent('FlameWatch Prediction Results');
    window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}`, '_blank');
}

function shareOnTwitter() {
    const url = encodeURIComponent(window.location.href);
    const text = encodeURIComponent('Check out this wildfire prediction from FlameWatch!');
    window.open(`https://twitter.com/intent/tweet?url=${url}&text=${text}`, '_blank');
}

function shareOnInstagram() {
    // Instagram doesn't have a direct sharing API for web
    // Redirect to Instagram app/website
